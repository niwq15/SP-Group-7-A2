...
s3boxes <- array(rep(0,nreps*2*n*n), dim = c(2*n,n,nreps)) # create an array of the cards pulled from the boxes. Each matrix is a simulation
...
for (k in 1:nreps){ # for each simulation
  ...
  for (i in prisoner_numbers){ # for each prisoner
    s3boxes[i,,k] <- box_contains[sample(prisoner_numbers,n)]
  } # prisoner has selected all n boxes now, need to check if they found their card
  for (i in prisoner_numbers){ # for each prisoner
    if (i %in% s3boxes[i,,k]){ # check if their card number is in their row
